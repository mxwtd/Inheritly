...
const getAllAssets = async (req, res, next) => {
  ...
  try {
    ...
    const totalBalance = propertiesBalance + vehiclesBalance + jewelsBalance + stocksBalance + cryptosBalance + bondsBalance + commoditiesBalance + fundsBalance
    const totalAssets = properties.length + vehicles.length + jewels.length + stocks.length + cryptos.length + bonds.length + commodities.length + funds.length

    res.json({ totalBalance, totalAssets })
  } catch (error) {
    next(error)
  }
}
