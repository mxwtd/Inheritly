...
import { useUpdateEntityMutation, useGetEntityByIdQuery } from '../services/entitiesApiSlice'
...
const EditEntity = () => {
  ...
  const [date, setDate] = useState(() => {
    const originalDate = new Date(entity?.date)
    return formatDate(originalDate)
  })
  const [value, setValue] = useState(entity?.value || '')
  const [taxStatus, setTaxStatus] = useState(entity?.taxStatus || '')
  ...
  const [contactInformation, setContactInformation] = useState({
    accountNumber: entity?.contactInformation?.accountNumber || '',
    email: entity?.contactInformation?.email || '',
    phone: entity?.contactInformation?.phone || '',
    companyAddress: entity?.contactInformation?.companyAddress || ''
  } || {})

  const [photo, setPhoto] = useState(entity?.photo || null)
  const [files, setFiles] = useState(entity?.files || [])

  const [isPrivate, setIsPrivate] = useState(() => {
    if (entity?.contactInformation) {
      return false
    } else {
      return true
    }
  ...
  useEffect(() => {
    ...
    if (entity) {
      ...
      setDate(formatDate(new Date(entity.date)))
      setValue(entity.value)
      setTaxStatus(entity.taxStatus)
      setType(entity.type)
      setCity(entity.city)
  ...
  const onSaveEntityClicked = async (e) => {
    e.preventDefault()

    console.log('update button clicked')

    const entityData = new FormData(e.target)

    console.log('EntityData name: ', entityData.get('name'))

    console.log('')

    if (files) {
      entityData.delete('files')
      files.forEach(file => {
        if (file instanceof File) {
          entityData.append('files', file)
        }
      })
    }
  ...
  const content = (
    <Entities backTo={`/entities/${id}`}>
      ...
      <div className='bg-white backdrop-blur-md rounded-3xl shadow-xl dark:border md:mt-0 p-6 dark:bg-slate-800 dark:border-slate-700'>
        <div className='mb-10'>
          <h1 className='text-4xl font-semibold text-slate-800 dark:text-slate-100'>Edit {entity?.name}</h1>
        </div>
        <form encType='multipart/form-data' onSubmit={onSaveEntityClicked} className='space-y-4 md:space-y-6' action='#'>
          ...
          <div className='md:flex md:justify-between items-center bg-slate-100 dark:bg-slate-900 rounded-2xl p-5'>
            ...
            <div className='md:w-1/2 md:pr-2'>
              <div className='mb-3'>
                <FieldInput label='Entity name' value={name} onChange={onNameChanged} name='name' type='text' placeholder='Entity name' errors={errors} isRequire />
              </div>
              <div>
            ...
            <div className='md:w-1/2 md:pl-2'>
              ...
              <div>
                <FieldInput label='Value' value={value} onChange={onValueChanged} name='value' type='text' placeholder='Entity Value' errors={errors} isRequire />
              </div>
            </div>
          </div>
          ...
          <div className='bg-slate-100 dark:bg-slate-900 rounded-2xl p-5'>
            <div>
              <div className='flex flex-row justify-start items-center'>
                ...
                <h2 className='ml-3 text-lg font-medium text-gray-900 dark:text-gray-300'>Owned by an Entity?</h2>
              </div>
              {!isPrivate && (
                <>
          ...
          {files?.length > 0
            ? (
              <FilesList id={id} files={files} setFiles={setFiles} type='entities' />
              )
            : null}
