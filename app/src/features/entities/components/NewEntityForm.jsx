...
import { useAddNewEntityMutation } from '../services/entitiesApiSlice.js'
  ...
  const content = (
    <Entities backTo='/entities'>
      <div className='bg-white backdrop-blur-md rounded-3xl shadow-xl dark:border md:mt-0 p-6 dark:bg-slate-800 dark:border-slate-700'>
        <div className='mb-10'>
          <h1 className='text-4xl font-semibold text-slate-800 dark:text-slate-100'>Add an Entity</h1>
        </div>
        <form encType='multipart/form-data' onSubmit={onSaveEntityClicked} className='space-y-4 md:space-y-6' action='#'>
          ...
          <div className='bg-slate-100 dark:bg-slate-900 rounded-2xl p-5'>
            <h3 className='text-xl font-semibold text-slate-800 dark:text-slate-100 text-left mb-2'>Entity Information</h3>
          ...
          {files.length > 0
            ? (
              <FilesList files={files} setFiles={setFiles} type='entities' />
              )
            : null}
