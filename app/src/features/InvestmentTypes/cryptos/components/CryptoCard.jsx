...
const CryptoCard = ({ crypto }) => {
  return (
    <>
      <Link to={`./${crypto.id}`}>
        <div key={crypto.id} className='bg-white dark:bg-slate-800 shadow-lg rounded-xl'>
            ...
            <div className='h-full w-full'>
              ...
              <div className='mb-3 flex flex-col items-start justify-between px-1 md:items-start'>
                <div className='mb-2 w-full'>
                  <p className='text-lg font-bold text-slate-800 dark:text-slate-300 break-words overflow-hidden'>{crypto.name}</p>
                  <p className='mt-1 text-sm font-medium text-slate-600 dark:text-slate-400 md:mt-2'>{crypto.symbol}, {crypto.purchasedAt}</p>
                </div>
              </div>
              <div className='flex items-center justify-between md:items-center lg:justify-between '>
                <div className='flex'>
                  <p className='mb-0 pl-1 text-sm font-bold text-slate-600 dark:text-slate-400'>${formattedPrice(crypto.value)}</p>
                </div>
