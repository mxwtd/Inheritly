...
import { useGetFundByIdQuery } from '../services/fundsApiSlice.js'
...
const Fund = () => {
  ...
  if (isSuccess) {
      ...
      content = (
        <Funds backTo='/investments/funds'>
          <div className='flex justify-between items-center mb-5 md:mb-10'>
            <h1 className='text-3xl md:text-4xl font-bold text-slate-800 dark:text-white'>{fund?.name}</h1>
                ...
                <div className='h-full p-4'>
                  <div className='grid grid-cols-2 grid-rows-2 gap-4 xl:gap-8 text-slate-700 dark:text-white'>
                    <div className='bg-slate-200 dark:bg-slate-600 p-4 rounded-xl shadow-lg overflow-hidden flex flex-col justify-center'>
                      ...
                      <h3 className='text-md xl:text-xl text-slate-600 dark:text-slate-300'>{new Date(fund?.date).toLocaleDateString()}</h3>
                    </div>
                    <div className='bg-slate-200 dark:bg-slate-600 p-4 rounded-xl shadow-lg overflow-hidden flex flex-col justify-center'>
                    ...
                    <div className='bg-slate-200 dark:bg-slate-600 p-4 rounded-xl shadow-lg overflow-hidden flex flex-col justify-center'>
                      ...
                      <h3 className='text-md xl:text-xl text-slate-600 dark:text-slate-300'>{fund?.type}</h3>
                    </div>
                    <div className='bg-slate-200 dark:bg-slate-600 p-4 rounded-xl shadow-lg overflow-hidden flex flex-col justify-center'>
                      <h2 className='text-lg xl:text-2xl font-semibold'>Tax Status</h2>
                      <h3 className='text-md xl:text-xl text-slate-600 dark:text-slate-300'>{fund?.taxStatus}</h3>
                    </div>
                  </div>
          ...
          <div className='grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4'>
            <div className='rounded-xl bg-slate-50 aspect-w-1 aspect-h-2 col-span-1 lg:col-span-1 lg:row-span-2 dark:bg-slate-800 shadow-lg'>
              <div className='items-center'>
                <p className='text-xl lg:text-2xl px-4 pt-4 text-slate-400 dark:text-slate-500'>Fund Location</p>
              </div>
                      ...
                      <>
                        <div className='rounded-xl bg-slate-50 aspect-w-1 aspect-h-1 dark:bg-slate-800 shadow-lg p-4'>
                          <div className='p-4 bg-slate-200 dark:bg-slate-600 text-slate-700 dark:text-white rounded-xl shadow-lg h-full flex flex-col justify-center'>
                            <p className='text-xl lg:text-2xl px-4 mb-2'>Extra details</p>
                            <p className='text-md lg:text-lg px-4'>{fund?.description}</p>
                          </div>
                        </div>
                      </>
                      )
            ...
            {
              fund?.contactInformation
                ? (
                  <div className='rounded-xl bg-slate-50 aspect-w-1 aspect-h-1 dark:bg-slate-800 shadow-lg p-4'>
                    <div className='p-4 bg-slate-200 dark:bg-slate-600 text-slate-700 dark:text-white rounded-xl shadow-lg h-full flex flex-col justify-center'>
                      <p className='text-xl lg:text-2xl px-4 mb-2'>Contact</p>
                      {fund?.contactInformation?.accountNumber
                        ? <p className='text-md lg:text-lg px-4'>Account Number: {fund?.contactInformation.accountNumber}</p>
                        : null}
                      {fund?.contactInformation?.email
                        ? <p className='text-md lg:text-lg px-4'>Email: {fund?.contactInformation.email}</p>
                        : null}
                      {fund?.contactInformation?.phone
                        ? <p className='text-md lg:text-lg px-4'>Phone: {fund?.contactInformation.phone}</p>
                        : null}
                      {fund?.contactInformation?.companyAddress
                        ? <p className='text-md lg:text-lg px-4'>Company Address: {fund?.contactInformation.companyAddress}</p>
                        : null}
                    </div>
                  </div>
                  )
    ...
    return (
      <>
        {content}
        {showDeleteModal && <DeleteModal onClose={handleCloseDelete} investmentType={fund} />}
      </>
    )
  }
}
