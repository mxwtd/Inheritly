...
import { useGetStockByIdQuery } from '../services/stocksApiSlice.js'
...
const Stock = () => {
  ...
  if (isSuccess) {
    ...
    if (isSuccess) {
      ...
      content = (
        <Stocks backTo='/investments/stocks'>
          <div className='flex justify-between items-center mb-5 md:mb-10'>
            <h1 className='text-3xl md:text-4xl font-bold text-slate-800 dark:text-white'>{stock?.name}</h1>
                ...
                <div className='h-full p-4'>
                  <div className='grid grid-cols-2 grid-rows-2 gap-4 xl:gap-8 text-slate-700 dark:text-white'>
                    <div className='bg-slate-200 dark:bg-slate-600 p-4 rounded-xl shadow-lg overflow-hidden flex flex-col justify-center'>
                      ...
                      <h3 className='text-md xl:text-xl text-slate-600 dark:text-slate-300'>{new Date(stock?.date).toLocaleDateString()}</h3>
                    </div>
                    <div className='bg-slate-200 dark:bg-slate-600 p-4 rounded-xl shadow-lg overflow-hidden flex flex-col justify-center'>
                      <h2 className='text-lg xl:text-2xl font-semibold'>Value</h2>
                      <h3 className='text-md xl:text-xl text-slate-600 dark:text-slate-300'>{stock?.currency} {formattedPrice(stock?.value)}</h3>
                    </div>
                    <div className='bg-slate-200 dark:bg-slate-600 p-4 rounded-xl shadow-lg overflow-hidden flex flex-col justify-center'>
                      <h2 className='text-lg xl:text-2xl font-semibold'>Type</h2>
                      <h3 className='text-md xl:text-xl text-slate-600 dark:text-slate-300'>{stock?.type}</h3>
                    </div>
                    <div className='bg-slate-200 dark:bg-slate-600 p-4 rounded-xl shadow-lg overflow-hidden flex flex-col justify-center'>
                      <h2 className='text-lg xl:text-2xl font-semibold'>Tax Status</h2>
                      <h3 className='text-md xl:text-xl text-slate-600 dark:text-slate-300'>{stock?.taxStatus}</h3>
                    </div>
                  </div>
                              ...
                              <tr key={realIndex} className={realIndex % 2 === 0 ? 'cursor-pointer bg-white border-b dark:bg-slate-800 dark:border-slate-700' : 'cursor-pointer border-b bg-slate-50 dark:bg-slate-800 dark:border-slate-700'}>
                                ...
                                <td className='px-6 py-4'>
                                  <div className='flex items-center space-x-4'>
                                    <button onClick={() => handleRenameFile(realIndex)}>
                                    ...
                                    <button onClick={() => handleDeleteFile(realIndex)}>
                                      <svg className='w-5 h-5' fill='none' stroke='currentColor' strokeWidth='1.5' viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg' aria-hidden='true'>
                                        <path strokeLinecap='round' strokeLinejoin='round' d='M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0' />
                                      </svg>
                                    </button>
          ...
          <div className='grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4'>
            <div className='rounded-xl bg-slate-50 aspect-w-1 aspect-h-2 col-span-1 lg:col-span-1 lg:row-span-2 dark:bg-slate-800 shadow-lg'>
              <div className='items-center'>
                <p className='text-xl lg:text-2xl px-4 pt-4 text-slate-400 dark:text-slate-500'>Stock Location</p>
              </div>
            ...
            {
              (stock?.quantity || stock?.purchasePrice || stock?.purchasedAt || stock?.details || stock?.symbol)
                ? (
                  <>
                    <div className='rounded-xl bg-slate-50 aspect-w-1 aspect-h-1 dark:bg-slate-800 shadow-lg p-4'>
                      <div className='p-4 bg-slate-200 dark:bg-slate-600 text-slate-700 dark:text-white rounded-xl shadow-lg h-full flex flex-col justify-center'>
                        ...
                        {stock?.symbol
                          ? <p className='text-md lg:text-lg px-4'>Symbol: {stock?.symbol}</p>
                          : null}
                        {stock?.quantity
                          ? <p className='text-md lg:text-lg px-4'>Quantity: {stock?.quantity}</p>
                          : null}
                        {stock?.purchasePrice
                          ? <p className='text-md lg:text-lg px-4'>Purchase Price: {stock?.purchasePrice}</p>
                          : null}
                        {stock?.purchasedAt
                          ? <p className='text-md lg:text-lg px-4'>Purchased At: {stock?.purchasedAt}</p>
                          : null}
                        {stock?.details
                          ? <p className='text-md lg:text-lg px-4'>Details: {stock?.details}</p>
                          : null}
                      </div>
                    </div>
                  </>
            ...
            {
              stock?.contactInformation
                ? (
                  <div className='rounded-xl bg-slate-50 aspect-w-1 aspect-h-1 dark:bg-slate-800 shadow-lg p-4'>
                    <div className='p-4 bg-slate-200 dark:bg-slate-600 text-slate-700 dark:text-white rounded-xl shadow-lg h-full flex flex-col justify-center'>
                      <p className='text-xl lg:text-2xl px-4 mb-2'>Contact</p>
                      <p className='text-md lg:text-lg px-4'>Account Number: {stock?.contactInformation.accountNumber}</p>
                      <p className='text-md lg:text-lg px-4'>Email: {stock?.contactInformation.email}</p>
                      <p className='text-md lg:text-lg px-4'>Phone: {stock?.contactInformation.phone}</p>
                      <p className='text-md lg:text-lg px-4'>Company Address: {stock?.contactInformation.companyAddress}</p>
                    </div>
                  </div>
                  )
                : null
    ...
    return (
      <>
        {content}
        {showDeleteModal && <DeleteModal onClose={handleCloseDelete} investmentType={stock} />}
      </>
    )
  }
}
