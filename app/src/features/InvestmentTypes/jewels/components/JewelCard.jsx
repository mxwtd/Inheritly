...
const JewelCard = ({ jewel }) => {
  return (
    <>
      <Link to={`./${jewel.id}`}>
        <div key={jewel.id} className='bg-white dark:bg-slate-800 shadow-lg rounded-xl'>
            ...
            <div className='h-full w-full'>
              ...
              <div className='mb-3 flex flex-col items-start justify-between px-1 md:items-start'>
                <div className='mb-2 w-full'>
                  <p className='text-lg font-bold text-slate-800 dark:text-slate-300 break-words overflow-hidden'>{jewel.name}</p>
                </div>
              </div>
              <div className='flex items-center justify-between md:items-center lg:justify-between '>
                <div className='flex'>
                  <p className='mb-0 pl-1 text-sm font-bold text-slate-600 dark:text-slate-400'>${formattedPrice(jewel.value)}</p>
                </div>
